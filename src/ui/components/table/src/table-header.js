...
export default {
                    ...
                    <div class={ ['cell', column.filteredValue && column.filteredValue.length > 0 ? 'highlight' : '', column.labelClassName] }>
                      ...
                      }
                      {
                        column.filterable ? (<span
                          class="fm-table__column-filter-trigger"
                          on-click={ ($event) => this.handleFilterClick($event, column) }>
                          <i class={ ['fm-icon-arrow-down', column.filterOpened ? 'fm-icon-arrow-up' : ''] }></i>
                        </span>) : ''
                      }
                    </div>
                  </th>))
  ...
  mounted() {
    // nextTick 是有必要的 https://github.com/FmemeFE/element/pull/11311
    this.$nextTick(() => {
  ...
  methods: {
    ...
    handleFilterClick(event, column) {
      ...
      cell = cell.querySelector('.fm-table__column-filter-trigger') || cell
      ...
      if (!filterPanel) {
        ...
        !this.$isServer && filterPanel.$mount(document.createFmement('div'))
      }
    ...
    handleMouseDown(event, column) {
      ...
      if (this.draggingColumn && this.border) {
        ...
        const table = this.$parent
        const tableFm = table.$el
        const tableLeft = tableFm.getBoundingClientRect().left
        const columnFm = this.$el.querySelector(`th.${column.id}`)
        const columnRect = columnFm.getBoundingClientRect()
        const minLeft = columnRect.left - tableLeft + 30

        addClass(columnFm, 'noclick')
