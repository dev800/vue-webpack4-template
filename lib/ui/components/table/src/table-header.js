...
export default {
                    ...
                    <div class={['cell', column.filteredValue && column.filteredValue.length > 0 ? 'highlight' : '', column.labelClassName]}>
                      ...
                      }
                      {
                        column.filterable ? (<span
                          class="ui-table__column-filter-trigger"
  ...
  mounted () {
    // nextTick 是有必要的 https://github.com/UiemeFE/element/pull/11311
    this.$nextTick(() => {
  ...
  methods: {
    ...
    handleFilterClick (event, column) {
      ...
      cell = cell.querySelector('.ui-table__column-filter-trigger') || cell
      ...
      if (!filterPanel) {
        ...
        !this.$isServer && filterPanel.$mount(document.createUiement('div'))
      }
    ...
    handleMouseDown (event, column) {
      ...
      if (this.draggingColumn && this.border) {
        ...
        const table = this.$parent
        const tableUi = table.$el
        const tableLeft = tableUi.getBoundingClientRect().left
        const columnUi = this.$el.querySelector(`th.${column.id}`)
        const columnRect = columnUi.getBoundingClientRect()
        const minLeft = columnRect.left - tableLeft + 30

        addClass(columnUi, 'noclick')
