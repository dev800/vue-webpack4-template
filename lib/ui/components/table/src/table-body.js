...
export default {
    ...
    return (
      <table
        ...
        <tbody>
          ...
          <ui-tooltip effect={this.table.tooltipEffect} placement="top" ref="tooltip" content={this.tooltipContent}></ui-tooltip>
        </tbody>
      </table>
    )
  },
  ...
  watch: {
    ...
    // update DOM manually. see https://github.com/UiemeFE/element/pull/13954/files#diff-9b450c00d0a9dec0ffad5a3176972e40
    'store.states.hoverRow' (newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll('.ui-table__row')
  ...
  methods: {
    ...
    getRowClass (row, rowIndex) {
      ...
      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('ui-table__row--striped')
      }
    ...
    handleCellMouseEnter (event, row) {
      ...
      if (!(hasClass(cellChild, 'ui-tooltip') && cellChild.childNodes.length)) {
        return
      }
    ...
    rowRender (row, $index, treeRowData) {
      ...
      let display = true
      if (treeRowData) {
        rowClasses.push('ui-table__row--levui-' + treeRowData.level)
        display = treeRowData.display
      }
      // 指令 v-show 会覆盖 row-style 中 display
      // 使用 :style 代替 v-show https://github.com/UiemeFE/element/issues/16995
    ...
    wrappedRowRender (row, $index) {
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        if (!renderExpanded) {
          console.error('[Uiement Error]renderExpanded is required.')
          return tr
        }
        // 使用二维数组，避免修改 $index
        return [[
          ...
          <tr key={'expanded-row__' + tr.key}>
            <td colspan={this.columnsCount} class="ui-table__expanded-cell">
              {renderExpanded(this.$createUiement, { row, $index, store: this.store })}
            </td>
          </tr>]]
